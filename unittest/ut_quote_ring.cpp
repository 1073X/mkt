...
TEST(ut_quote_ring, resolve_size) {
    auto exp = sizeof(miu::mkt::quote_ring) + 128 * sizeof(miu::mkt::quote);
    EXPECT_EQ(exp, miu::mkt::quote_ring::resolve_size(128));
}

TEST(ut_quote_ring, default) {
    char buf[4096] {};
    auto ring = miu::mkt::quote_ring::make(buf, sizeof(buf), 8);
    ASSERT_NE(nullptr, ring);
    EXPECT_EQ(8U, ring->capacity());
    EXPECT_EQ(0U, ring->index());
    EXPECT_EQ(miu::ref::symbol {}, ring->symbol());
    EXPECT_EQ(0U, ring->instrument_id());
...
TEST(ut_quote_ring, set_value) {
    ...
    ring->set_instrument_id(12);
    EXPECT_EQ(12U, ring->instrument_id());
